...
| POST | `/shipments` | `shipment.create` | `{customerId, tariffId, originWarehouseId?, destinationWarehouseId?, insuranceAmount?, penerimaName?, penerimaAddress?, penerimaContact?}` — **insuranceAmount** is an optional manually entered Rupiah amount; **tariffId** comes from the route dropdown (filtered by customer B2B/B2C). Origin/destination are auto-filled from the tariff and `tariffId` stored on the shipment. Penerima (recipient) is printed on both resi types. Legacy `{origin, destination}` still accepted. → code `MKT-000NNN` + resi + tracking `CREATED` |
...
| PUT | `/shipments/{id}` | `shipment.update` | Update master fields (while editable) — accepts `tariffId` to change the route, `insuranceAmount` in Rupiah, and `penerimaName/penerimaAddress/penerimaContact` |
