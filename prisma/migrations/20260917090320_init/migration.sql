...
CREATE TABLE "MasterShipment" (
    ...
    -- Revise round 8 — Fulfillment Mode. "STANDARD" = kurir → gudang →
    -- transport → gudang → kurir delivery flow; "DIRECT" = driver picks up
    -- directly at the origin and delivers directly to the destination,
    -- bypassing the company warehouse scan flow. Defaults to STANDARD so
    -- existing rows are unaffected. This column was missing from the
    -- original migration SQL (the schema had it, but the migration file
    -- was not regenerated after the schema change). Without it, the
    -- seeder's DIRECT demo shipment (MKT-000009) fails to insert.
...
CREATE TABLE "DetailShipment" (
    ...
    -- directly instead of computing L×W×H/1.000.000. Use case: irregularly-
    -- shaped packages where entering dimensions is impractical. This
    -- column was missing from the original migration SQL (the schema had
    -- it, but the migration file was not regenerated). Without it, the
    -- shipments API / pricing helper fails when this field is set.
...
CREATE TABLE "Vehicle" (
    ...
    "createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    "updatedAt" TIMESTAMP(3) NOT NULL,
    -- Revise round 9 — physical cargo box dimensions in meters. When all
    -- three are set, maxVolumeM3 is auto-computed as L × W × H. These
    -- columns were missing from the original migration SQL (the schema
    -- had them, but the migration file was not regenerated after the
    -- schema change). Without them, PostgreSQL rejects vehicle creation
    -- because the API always sends them.
